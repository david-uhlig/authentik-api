...
module Authentik::Api
  ...
  class KubernetesServiceConnection < ApiModelBase
    attr_accessor :pk
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Authentik::Api::KubernetesServiceConnection` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Authentik::Api::KubernetesServiceConnection`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [pk, name, local, component, verbose_name, verbose_name_plural, meta_model_name, kubeconfig, verify_ssl].hash
    end

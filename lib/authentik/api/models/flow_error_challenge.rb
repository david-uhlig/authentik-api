...
module Authentik::Api
  ...
  class FlowErrorChallenge < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Authentik::Api::FlowErrorChallenge` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Authentik::Api::FlowErrorChallenge`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'component')
        ...
        self.component = 'ak-stage-flow-error'
      end
      ...
      if attributes.key?(:'traceback')
        self.traceback = attributes[:'traceback']
      end
    end
    ...
    def hash
      [flow_info, component, response_errors, request_id, error, traceback].hash
    end

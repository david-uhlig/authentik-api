...
module Authentik::Api
  ...
  class PromptStage < ApiModelBase
    attr_accessor :pk
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Authentik::Api::PromptStage` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Authentik::Api::PromptStage`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'validation_policies')
        if (value = attributes[:'validation_policies']).is_a?(Array)
          self.validation_policies = value
        end
      end
    end
    ...
    def hash
      [pk, name, component, verbose_name, verbose_name_plural, meta_model_name, flow_set, fields, validation_policies].hash
    end

...
module Authentik::Api
  ...
  class ReputationPolicy < ApiModelBase
    attr_accessor :pk
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Authentik::Api::ReputationPolicy` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Authentik::Api::ReputationPolicy`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def threshold=(threshold)
      ...
      if threshold > 2147483647
        fail ArgumentError, 'invalid value for "threshold", must be smaller than or equal to 2147483647.'
      end

      if threshold < -2147483648
        fail ArgumentError, 'invalid value for "threshold", must be greater than or equal to -2147483648.'
      end

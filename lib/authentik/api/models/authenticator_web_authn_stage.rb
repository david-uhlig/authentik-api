...
module Authentik::Api
  ...
  class AuthenticatorWebAuthnStage < ApiModelBase
    attr_accessor :pk
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Authentik::Api::AuthenticatorWebAuthnStage` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Authentik::Api::AuthenticatorWebAuthnStage`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'device_type_restrictions_obj')
        if (value = attributes[:'device_type_restrictions_obj']).is_a?(Array)
          self.device_type_restrictions_obj = value
        end
      else
    ...
    def list_invalid_properties
      ...
      if @device_type_restrictions_obj.nil?
    ...
    # Custom attribute writer method with validation
    # @param [Object] device_type_restrictions_obj Value to be assigned
    def device_type_restrictions_obj=(device_type_restrictions_obj)
      if device_type_restrictions_obj.nil?
        fail ArgumentError, 'device_type_restrictions_obj cannot be nil'
      end

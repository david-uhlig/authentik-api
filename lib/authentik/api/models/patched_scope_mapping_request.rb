...
module Authentik::Api
  ...
  class PatchedScopeMappingRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Authentik::Api::PatchedScopeMappingRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Authentik::Api::PatchedScopeMappingRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@scope_name.nil? && @scope_name.to_s.length < 1
        invalid_properties.push('invalid value for "scope_name", the character length must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if !@scope_name.nil? && @scope_name.to_s.length < 1
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] scope_name Value to be assigned
    def scope_name=(scope_name)
      if scope_name.nil?
        fail ArgumentError, 'scope_name cannot be nil'
      end

      if scope_name.to_s.length < 1
        fail ArgumentError, 'invalid value for "scope_name", the character length must be greater than or equal to 1.'
      end
    ...
    def hash
      [managed, name, expression, scope_name, description].hash
    end

...
module Authentik::Api
  ...
  class PatchedMicrosoftEntraProviderRequest < ApiModelBase
    attr_accessor :name
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Authentik::Api::PatchedMicrosoftEntraProviderRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Authentik::Api::PatchedMicrosoftEntraProviderRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@tenant_id.nil? && @tenant_id.to_s.length < 1
    ...
    def valid?
      ...
      return false if !@tenant_id.nil? && @tenant_id.to_s.length < 1
    ...
    def tenant_id=(tenant_id)
      ...
      if tenant_id.to_s.length < 1
        fail ArgumentError, 'invalid value for "tenant_id", the character length must be greater than or equal to 1.'
      end

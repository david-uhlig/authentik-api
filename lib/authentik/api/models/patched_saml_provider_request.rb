...
module Authentik::Api
  ...
  class PatchedSAMLProviderRequest < ApiModelBase
    attr_accessor :name
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Authentik::Api::PatchedSAMLProviderRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Authentik::Api::PatchedSAMLProviderRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if !@acs_url.nil? && @acs_url.to_s.length > 200
    ...
    def acs_url=(acs_url)
      ...
      if acs_url.to_s.length < 1
        fail ArgumentError, 'invalid value for "acs_url", the character length must be greater than or equal to 1.'
      end
    ...
    def hash
      [name, authentication_flow, authorization_flow, invalidation_flow, property_mappings, acs_url, audience, issuer, assertion_valid_not_before, assertion_valid_not_on_or_after, session_valid_not_on_or_after, name_id_mapping, digest_algorithm, signature_algorithm, signing_kp, verification_kp, encryption_kp, sign_assertion, sign_response, sp_binding, default_relay_state].hash
    end

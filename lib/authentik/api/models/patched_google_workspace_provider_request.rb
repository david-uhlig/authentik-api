...
module Authentik::Api
  ...
  class PatchedGoogleWorkspaceProviderRequest < ApiModelBase
    attr_accessor :name
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Authentik::Api::PatchedGoogleWorkspaceProviderRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Authentik::Api::PatchedGoogleWorkspaceProviderRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@default_group_email_domain.nil? && @default_group_email_domain.to_s.length < 1
    ...
    def valid?
      ...
      return false if !@delegated_subject.nil? && @delegated_subject.to_s.length > 254
      return false if !@delegated_subject.nil? && @delegated_subject.to_s.length < 1
      return false if !@scopes.nil? && @scopes.to_s.length < 1
      return false if !@default_group_email_domain.nil? && @default_group_email_domain.to_s.length < 1

...
module Authentik::Api
  ...
  class PatchedSCIMProviderRequest < ApiModelBase
    attr_accessor :name
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Authentik::Api::PatchedSCIMProviderRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Authentik::Api::PatchedSCIMProviderRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@service_provider_config_cache_timeout.nil? && @service_provider_config_cache_timeout.to_s.length < 1
        invalid_properties.push('invalid value for "service_provider_config_cache_timeout", the character length must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if !@service_provider_config_cache_timeout.nil? && @service_provider_config_cache_timeout.to_s.length < 1
    ...
    # Custom attribute writer method with validation
    # @param [Object] service_provider_config_cache_timeout Value to be assigned
    def service_provider_config_cache_timeout=(service_provider_config_cache_timeout)
      if service_provider_config_cache_timeout.nil?
        fail ArgumentError, 'service_provider_config_cache_timeout cannot be nil'
      end

      if service_provider_config_cache_timeout.to_s.length < 1
        fail ArgumentError, 'invalid value for "service_provider_config_cache_timeout", the character length must be greater than or equal to 1.'
      end
    ...
    def hash
      [name, property_mappings, property_mappings_group, url, verify_certificates, token, auth_mode, auth_oauth, auth_oauth_params, compatibility_mode, service_provider_config_cache_timeout, exclude_users_service_account, filter_group, sync_page_size, sync_page_timeout, dry_run].hash
    end

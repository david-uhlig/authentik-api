...
module Authentik::Api
  ...
  class PatchedOAuth2ProviderRequest < ApiModelBase
    attr_accessor :name
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Authentik::Api::PatchedOAuth2ProviderRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Authentik::Api::PatchedOAuth2ProviderRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] access_code_validity Value to be assigned
    def access_code_validity=(access_code_validity)
      if access_code_validity.nil?
        fail ArgumentError, 'access_code_validity cannot be nil'
      end

      if access_code_validity.to_s.length < 1
        fail ArgumentError, 'invalid value for "access_code_validity", the character length must be greater than or equal to 1.'
      end

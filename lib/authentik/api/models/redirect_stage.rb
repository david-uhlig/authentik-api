...
module Authentik::Api
  ...
  class RedirectStage < ApiModelBase
    attr_accessor :pk
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Authentik::Api::RedirectStage` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Authentik::Api::RedirectStage`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'keep_context')
        self.keep_context = attributes[:'keep_context']
      end
      ...
      if attributes.key?(:'target_static')
        self.target_static = attributes[:'target_static']
      end

      if attributes.key?(:'target_flow')
        self.target_flow = attributes[:'target_flow']
      end
    end
    ...
    def hash
      [pk, name, component, verbose_name, verbose_name_plural, meta_model_name, flow_set, keep_context, mode, target_static, target_flow].hash
    end

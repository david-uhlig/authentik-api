...
module Authentik::Api
  ...
  class PatchedGeoIPPolicyRequest < ApiModelBase
    attr_accessor :name
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Authentik::Api::PatchedGeoIPPolicyRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Authentik::Api::PatchedGeoIPPolicyRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@countries.nil? && @countries.length > 249
    ...
    def valid?
      ...
      return false if !@countries.nil? && @countries.length > 249
    ...
    def hash
      [name, execution_logging, asns, countries, check_history_distance, history_max_distance_km, distance_tolerance_km, history_login_count, check_impossible_travel, impossible_tolerance_km].hash
    end

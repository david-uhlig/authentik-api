...
module Authentik::Api
  # Settings Serializer
  class PatchedSettingsRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Authentik::Api::PatchedSettingsRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Authentik::Api::PatchedSettingsRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@avatars.nil? && @avatars.to_s.length < 1
    ...
    # @param [Object] avatars Value to be assigned
    def avatars=(avatars)
      if avatars.nil?
        fail ArgumentError, 'avatars cannot be nil'
      end

      if avatars.to_s.length < 1
        fail ArgumentError, 'invalid value for "avatars", the character length must be greater than or equal to 1.'
      end
    ...
    def event_retention=(event_retention)
      if event_retention.nil?
        fail ArgumentError, 'event_retention cannot be nil'
      end

      if event_retention.to_s.length < 1
        fail ArgumentError, 'invalid value for "event_retention", the character length must be greater than or equal to 1.'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] default_token_duration Value to be assigned
    def default_token_duration=(default_token_duration)
      if default_token_duration.nil?
        fail ArgumentError, 'default_token_duration cannot be nil'
      end

      if default_token_duration.to_s.length < 1
        fail ArgumentError, 'invalid value for "default_token_duration", the character length must be greater than or equal to 1.'
      end

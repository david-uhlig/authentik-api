...
module Authentik::Api
  ...
  class GroupLDAPSourceConnection < ApiModelBase
    attr_accessor :pk
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Authentik::Api::GroupLDAPSourceConnection` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Authentik::Api::GroupLDAPSourceConnection`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] source_obj Value to be assigned
    def source_obj=(source_obj)
      if source_obj.nil?
        fail ArgumentError, 'source_obj cannot be nil'
      end
    ...
    def hash
      [pk, group, source, source_obj, identifier, created, last_updated, group_obj].hash
    end

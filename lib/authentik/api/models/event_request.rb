  ...
  class EventRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Authentik::Api::EventRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Authentik::Api::EventRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if @app.to_s.length < 1
    ...
    def app=(app)
      ...
      if app.to_s.length < 1
        fail ArgumentError, 'invalid value for "app", the character length must be greater than or equal to 1.'
      end
    ...
    def client_ip=(client_ip)
      if !client_ip.nil? && client_ip.to_s.length < 1
        fail ArgumentError, 'invalid value for "client_ip", the character length must be greater than or equal to 1.'
      end
